...
ssf.Frame.init()
ui_until.print("开始录制 按ctrl+5 停止录制")
...
listener.addListener(function (e, down) {
  ...
  if (e.state == "DOWN" && (down["MOUSE LEFT"])) {
    ...
    try {
      ...
      const pos_info = ssf.Input.mouse_pos()
      Deno.writeTextFileSync(fileName, `ssf.Input.move(${pos_info.x},${pos_info.y},ssf.enums.Coordinate.Abs) \r\n`, { append: true });
      Deno.writeTextFileSync(fileName, `ssf.Input.button(ssf.enums.Button.Left,ssf.enums.Direction.Click)\r\n`, { append: true });
